...
st.code('old = df.copy() #We will need it later')
old = df.copy()
...
for i in range(len(num_cols)):
    axis[i].hist(df[num_cols[i]], bins=9)
    axis[i].set_title(num_cols[i])
axis[0].set_ylabel('Number of clients')
st.pyplot(fig)
...
st.code('fig = sns.pairplot(data=df[num_cols+[target_col]],hue=target_col)')
...
st.code("""
    ...
    title = ['Month-to-month', 'One year', 'Two years']
""")
df1 = df[df['Sex'] == 1]
df2 = df[df['Sex'] == 0]
cond = [df['ContractDuration'] == 0, df['ContractDuration'] == 1, df['ContractDuration'] == 2]
...
st.code("""
    ...
    for j in range(3):
        ...
        axis[j].set_title(title[j])
""")
fig, axis = plt.subplots(1, 3, figsize=(20, 5))
for j in range(3):
    axis[j].hist([df1[cond[j]]['ClientPeriod'], df2[cond[j]]['ClientPeriod']], bins=9)
    axis[j].legend(['Male', 'Female'])
    axis[j].set_xlabel('ContractDuration')
    axis[j].set_ylabel('Number of clients')
    axis[j].set_title(title[j])
st.pyplot(fig)
...
st.code("""
    ...
    ax.set_xlabel('NumberOfServices')
...
st.subheader('''
    The hypotesis was partly confirmd. 
    We knew that function showing ratio of left depending on NumberOfServices has form of mountine with 
    pike at 3 purchaced surveces. Young and Married people are actually 
